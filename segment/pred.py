...
class Prediction():
    ...
    def run(self, frame):
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                # print(f'proto type : {type(proto[2])}, shape : {proto[2].shape}')
                ...
                if self.save_txt:
                    ...
                    print(f'segments : {len(segments), segments[0].shape}')
                ...
                for j, (*xyxy, conf, cls) in enumerate(reversed(det[:, :6])):
                    ...
                    # print(f'(boxes : {xyxy}, conf : {conf}, cls : {cls}')

                    if save_img or self.save_crop :  # Add bbox to image
                ...
                masks = None
                segments = None
                # print(f'no detections, {det.shape}')
                # return  None
            # Stream results
            im0 = annotator.result()
            # print(f'final frame shape : {im0.shape}')
            
            # print(f'masks : {type(masks), masks.shape}; im0 : {im0.shape}')
            ...
            if save_img:
                p = 'output.jpg'
                cv2.imwrite(str(save_dir / p), im0)
